...
def _extract_text_from_pdf(pdf_data, logger):
    ...
    try:
        ...
        # 🔇 DESHABILITAR LOGS VERBOSOS DE PDFMINER
        logging.getLogger('pdfminer').setLevel(logging.ERROR)
        logging.getLogger('pdfplumber').setLevel(logging.ERROR)

        # Configuración
        MAX_PDF_SIZE_MB = 5  # Reducido a 5MB
        MAX_PAGES = 20  # Reducido a 20 páginas
        TIMEOUT_FIRST_PAGE = 5  # 5 segundos para la primera página (prueba rápida)
        TIMEOUT_PER_PAGE = 8
        TOTAL_TIMEOUT = 45

        # Validar tamaño del PDF
        pdf_size_mb = len(pdf_data) / (1024 * 1024)
        logger.info(f"📊 Tamaño del PDF: {pdf_size_mb:.2f}MB")

        if pdf_size_mb > MAX_PDF_SIZE_MB:
            logger.warning(f"⚠️ PDF muy grande ({pdf_size_mb:.2f}MB) → Usando OCR directo")
        ...
        try:
            logger.info(f"📖 Intentando extracción con pdfplumber...")
            ...
            with pdfplumber.open(pdf_file) as pdf:
                ...
                if total_pages > MAX_PAGES:
                    logger.warning(f"⚠️ Demasiadas páginas ({total_pages}) → Usando OCR")
                    return _extract_text_with_ocr(pdf_data, logger)

                # 🚀 PRUEBA RÁPIDA: Intentar primera página con timeout corto
                logger.info("🔬 Probando primera página...")
                with ThreadPoolExecutor(max_workers=1) as executor:
                    try:
                        future = executor.submit(pdf.pages[0].extract_text)
                        first_page_text = future.result(timeout=TIMEOUT_FIRST_PAGE)

                        # Si la primera página no tiene texto o es muy lenta, usar OCR
                        if not first_page_text or len(first_page_text.strip()) < 20:
                            logger.warning("⚠️ Primera página sin texto suficiente → OCR")
                            return _extract_text_with_ocr(pdf_data, logger)

                        text += first_page_text + "\n"
                        logger.info(f"✅ Primera página OK ({len(first_page_text)} caracteres)")

                    except FutureTimeoutError:
                        logger.warning(f"⏱️ Primera página tardó >{TIMEOUT_FIRST_PAGE}s → PDF complejo, usando OCR")
                        return _extract_text_with_ocr(pdf_data, logger)
                    except Exception as e:
                        logger.warning(f"⚠️ Error en primera página: {str(e)[:100]} → OCR")
                        return _extract_text_with_ocr(pdf_data, logger)

                # Si llegamos aquí, la primera página fue exitosa, continuar con el resto
                if total_pages > 1:
                    logger.info(f"📖 Procesando {total_pages - 1} páginas restantes...")

                    failed_pages = 0
                    max_failed = 2  # Reducido a 2
                    ...
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        for page_num in range(1, total_pages):
                            ...
                            if elapsed > TOTAL_TIMEOUT:
                                logger.warning(f"⏱️ Timeout total ({TOTAL_TIMEOUT}s) → OCR")
                                return _extract_text_with_ocr(pdf_data, logger)

                            try:
                                logger.debug(f"   Página {page_num + 1}/{total_pages}...")

                                future = executor.submit(pdf.pages[page_num].extract_text)
                                page_text = future.result(timeout=TIMEOUT_PER_PAGE)
                                ...
                                logger.warning(f"⏱️ Timeout página {page_num + 1}")
                                failed_pages += 1
                            except Exception as e:
                                logger.debug(f"⚠️ Error página {page_num + 1}")
                                failed_pages += 1

                            # Si fallan páginas, cambiar a OCR
                            if failed_pages >= max_failed:
                                logger.warning(f"⚠️ {failed_pages} páginas fallidas → OCR")
                                return _extract_text_with_ocr(pdf_data, logger)

            if text.strip():
                elapsed = time.time() - start_time
                logger.info(f"✅ Extracción completada ({len(text)} caracteres en {elapsed:.1f}s)")
                return text
            else:
                logger.warning("⚠️ No se extrajo texto → OCR")
                return _extract_text_with_ocr(pdf_data, logger)

        except Exception as e:
            logger.warning(f"⚠️ Error con pdfplumber: {str(e)[:100]} → OCR")
            return _extract_text_with_ocr(pdf_data, logger)
    ...
    def process_email(self, email_data, logger):
        ...
        try:
            ...
            if result['success']:
                ...
                # Agregar consultar_reparacion a extracted_data para usuarios CC
                if extracted_data and result.get('consultar_reparacion'):
                    extracted_data['consultar_reparacion'] = result.get('consultar_reparacion')
