...
class IfrProRepository(IApiIfrProRepository):
    ...
    async def listar_garantias(
        ...
        """
        Obtiene las garantías para un tipo de preingreso

        Args:
            tipo_preingreso_id: ID del tipo de preingreso

        Returns:
            ApiResponse con las garantías disponibles
        """
        self.logger.info(
            "Listar garantías",
            tipo_preingreso_id=tipo_preingreso_id
        )

        # Construir endpoint
        endpoint = Endpoint(
            path=f"/v1/preingreso/garantia/{tipo_preingreso_id}",
            method=RequestMethod.GET,
            base_url=self.base_url
        )
